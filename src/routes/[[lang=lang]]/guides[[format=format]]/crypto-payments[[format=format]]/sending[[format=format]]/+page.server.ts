...
export async function load({ parent }) {
	...
	return {
		lite: false,
		title: await fetchLocaleString('ui.guides.pages.crypto-payments-sending', lang),
		nav: guidesNav(lang),
		sections: await Promise.all([
			fetchLocaleFragment('ui/guides/crypto-payments/sending', lang).then(renderFragmentToHtml),
		])
	};
}
