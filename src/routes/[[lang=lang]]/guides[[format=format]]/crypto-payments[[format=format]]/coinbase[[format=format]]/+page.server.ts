...
export async function load({ parent }) {
	...
	return {
		lite: false,
		title: await fetchLocaleString('ui.guides.pages.crypto-payments-coinbase', lang),
		nav: guidesNav(lang),
		sections: await Promise.all([
			fetchLocaleFragment('ui/guides/crypto-payments/coinbase', lang).then(renderFragmentToHtml),
		])
	};
}
