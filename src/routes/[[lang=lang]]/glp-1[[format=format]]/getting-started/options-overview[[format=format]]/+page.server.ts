...
export async function load({ parent }) {
	...
	return {
		title: await fetchLocaleString('ui.glp1.pages.getting-started-options-overview', lang),
		nav: glp1Nav(lang),
		sections: await Promise.all([
  		fetchLocaleFragment('ui/glp1/getting-started/options-overview', lang).then(renderFragmentToHtml)
		])
	};
}
