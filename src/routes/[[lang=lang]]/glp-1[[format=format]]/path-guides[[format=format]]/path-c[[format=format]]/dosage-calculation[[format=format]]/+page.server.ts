...
export async function load({ parent }) {
	...
	return {
		title: await fetchLocaleString('ui.glp1.pages.path-c-dosage-calculation', lang),
		nav: glp1Nav(lang),
		sections: await Promise.all([
  		fetchLocaleFragment('ui/glp1/path-guides/path-c/dosage-calculation', lang).then(renderFragmentToHtml)
		])
	};
}
