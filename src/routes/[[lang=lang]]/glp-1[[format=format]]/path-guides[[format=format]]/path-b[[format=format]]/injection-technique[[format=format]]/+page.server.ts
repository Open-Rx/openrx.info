...
export async function load({ parent }) {
	...
	return {
		title: await fetchLocaleString('ui.glp1.pages.path-b-injection-technique', lang),
		nav: glp1Nav(lang),
		sections: await Promise.all([
  		fetchLocaleFragment('ui/guides/subcutaneous-injection', lang).then(renderFragmentToHtml)
		])
	};
}
