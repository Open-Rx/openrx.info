...
export async function load({ parent }) {
	...
	return {
		title: await fetchLocaleString('ui.glp1.pages.path-b-authenticity', lang),
		nav: glp1Nav(lang),
		sections: await Promise.all([
  		fetchLocaleFragment('ui/glp1/path-guides/path-b/authenticity', lang).then(renderFragmentToHtml)
		])
	};
}
