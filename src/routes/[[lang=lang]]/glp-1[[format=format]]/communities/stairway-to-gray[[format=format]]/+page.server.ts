...
export async function load({ parent }) {
	...
	return {
		lite: false,
		title: await fetchLocaleString('ui.glp1.pages.communities-stairway-to-gray', lang),
		nav: glp1Nav(lang),
		sections: await Promise.all([
			fetchLocaleFragment('ui/glp1/communities/stairway-to-gray', lang).then(renderFragmentToHtml),
		])
	};
}
