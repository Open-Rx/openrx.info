...
export async function load({ parent }) {
	...
	return {
		lite: false,
		title: await fetchLocaleString('ui.glp1.pages.sourcing-ancillary-supplies', lang),
		nav: glp1Nav(lang),
		sections: await Promise.all([
			fetchLocaleFragment('ui/glp1/sourcing/ancillary-supplies', lang).then(renderFragmentToHtml),
		])
	};
}
