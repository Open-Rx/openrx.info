...
export async function load({ parent }) {
	...
	return {
		lite: false,
		title: await fetchLocaleString('ui.glp1.pages.sourcing-crypto-payments', lang),
		nav: glp1Nav(lang),
		sections: await Promise.all([
			fetchLocaleFragment('ui/guides/crypto-payments/overview', lang).then(renderFragmentToHtml),
		])
	};
}
