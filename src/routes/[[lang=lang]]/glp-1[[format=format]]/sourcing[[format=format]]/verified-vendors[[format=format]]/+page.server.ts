...
export async function load({ parent }) {
	...
	return {
		lite: false,
		title: await fetchLocaleString('ui.glp1.pages.sourcing-verified-vendors', lang),
		nav: glp1Nav(lang),
		sections: await Promise.all([
			fetchLocaleFragment('ui/glp1/sourcing/verified-vendors', lang).then(renderFragmentToHtml),
		])
	};
}
