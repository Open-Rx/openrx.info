...
export async function load({ parent }) {
	...
	return {
		title: await fetchLocaleString('ui.glp1.pages.index', lang),
		nav: glp1Nav(lang),
		sections: await Promise.all([
  		fetchLocaleFragment('ui/glp1/index/intro', lang).then(renderFragmentToHtml),
  		fetchLocaleFragment('ui/glp1/index/how-to-navigate', lang).then(renderFragmentToHtml)
		])
	};
}
