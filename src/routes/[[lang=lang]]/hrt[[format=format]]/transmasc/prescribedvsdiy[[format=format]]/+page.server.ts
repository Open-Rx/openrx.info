...
export async function load({ parent }) {
	...
	return {
		lite: false,
		title: await fetchLocaleString('ui.hrt.pages.transmasc-prescribedvsdiy', lang),
		nav: hrtNav(lang),
		sections: await Promise.all([
		fetchLocaleFragment('ui/hrt/transmasc/prescribedvsdiy/content', lang).then(renderFragmentToHtml)
		])
	};
}
