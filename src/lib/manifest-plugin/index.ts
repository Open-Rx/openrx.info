...
function generateFragmentType(root: string) {
  const files = glob.sync('locale/en/**/*.md', { cwd: root });
  const keys = files.map(f =>
    f.replace('locale/en/', '').replace(/\.md$/, '')
  );
  const union = keys.map(k => `  | '${k}'`).join('\n');
  writeFileSync(
    resolve(root, 'src/lib/manifest-plugin/fragment-type.generated.ts'),
    `// auto-generated by manifest plugin — do not edit\nexport type FragmentKey =\n${union};\n`
  );
}

function generateLangs(root: string) {
  const langs = readdirSync(resolve(root, 'locale'), { withFileTypes: true })
    .filter(d => d.isDirectory())
    .map(d => d.name);
  writeFileSync(
    resolve(root, 'src/lib/manifest-plugin/langs.generated.ts'),
...
export function manifestPlugin(options: { supportedLanguages: string[] }): Plugin {
  ...
  function scanAndGenerate() {
    ...
    const end = Date.now()
    console.log(`Generated ${files.length} pages from manifests in ${end - start}ms`)
  }
